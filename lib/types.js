...
const {
  readPointer: readPointerImpl,
  writePointer: writePointerImpl,
  readByteArray: readByteArrayImpl,
  readU8: readU8Impl,
...
const strlen = new NativeFunction(Module.getExportByName(null, 'strlen'), 'int', ['pointer']);
...
module.exports = {
  register() {
    ...
    const nativePointerProto = NativePointer.prototype;
    ...
    .forEach(spec => {
      ...
      global[alias] = makeType({
        ...
        read: makeReadImpl(nativePointerProto['read' + memoryType]),
        write: makeWriteImpl(nativePointerProto['write' + memoryType])
      });
    });

    global.RTLD_DEFAULT = detectRtldDefault();
    const _dlsym = new NativeFunction(Module.getExportByName(null, 'dlsym'), 'pointer', ['pointer', 'pointer']);
...
function makeReadImpl(impl) {
  return function (address) {
    return impl.call(address);
  };
}

function makeWriteImpl(impl) {
  return function (address, value) {
    impl.call(address, value);
  };
}
...
function readPointer(pointer) {
  const address = readPointerImpl.call(pointer);
  if (address.isNull())
    return null;
...
function writePointer(pointer, value) {
  writePointerImpl.call(pointer, toNativePointer(value));
}
...
function readCString(address) {
  ...
  const bytes = new Uint8Array(readByteArrayImpl.call(address, length));
  const result = [];
...
function readChar(address) {
  const v = readU8Impl.call(address);
  if (v == 0)
    return '\\0';
...
function detectRtldDefault() {
  ...
  const isAndroid = Module.findExportByName('libc.so', '__system_property_get') !== null;
  if (isAndroid) {
    return (Process.pointerSize === 8) ? NULL : ptr('-1');
  }
