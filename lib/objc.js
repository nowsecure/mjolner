...
function registerTypes() {
  ...
  free = new NativeFunction(Module.getExportByName(null, 'free'), 'void', ['pointer']);
}

function registerStubs() {
  global.choose = throwNotAvailable;
  global.objc_msgSend = throwNotAvailable;
}
...
function writeObject(pointer, value) {
  writePointerImpl.call(pointer, toNativeObject(value));
}
...
function readSelector(pointer) {
  const sel = readPointerImpl.call(pointer);
  if (sel.isNull())
    return null;
...
function writeSelector(pointer, value) {
  writePointerImpl.call(pointer, toNativeSelector(value));
}
...
function Instance(impl) {
  ...
  if (isInstance) {
    impl.retain();
    Script.bindWeak(impl, releaseObject.bind(impl.handle));
  }
...
function Constructor(impl) {
  const ctor = function () {
    ...
    Script.bindWeak(instance, releaseObject.bind(instance.handle));
    return instance;
  };
